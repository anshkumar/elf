...
concatenated = {}
for st in state.values():
    concatenated[st] = np.concatenate((x_batches[st],y_batches[st]), axis = 2)
    np.random.shuffle(concatenated[st])
    x_batches[st], y_batches[st] = np.split(concatenated[st], 2, axis = 2)
